...
mf_filekey = ('102024023515_0_estimates.npz','wn_snradjust_lattice')
lorentz_filekey = ('102024023515_0_estimates.npz','lorentz')
ifreg_filekey = ('100825035305_estimates_100825035305_seeds_mf.npz','matchedfilter2d_optim')
...
measprop = fmcw_sys.import_meas_prop_from_config(utils.PARAMS_PATH, str(results['param_name']))
sample_rate = measprop.sample_rate
lambd_c = measprop.lambd_c
gamma = measprop.bandwidth / measprop.Tchirp
freqsub = estimate_arr[:,:,:,0] / (1 / 2 * sample_rate / gamma * 3e8 / 2)
freqsum = estimate_arr[:,:,:,1] / (1 / 2 * sample_rate * lambd_c / 2)
freq1 = (freqsum + freqsub) / 2
freq2 = (freqsum - freqsub) / 2
...
estimate_arr[:,:,:,0] = (freq1-freq2) / 2 * sample_rate / gamma * 3e8 / 2
estimate_arr[:,:,:,1] = (freq1+freq2) / 2 * sample_rate * lambd_c / 2
...
ax1.axis('off')
ax1.set_title('(i) Lorentzian Fitting', fontsize=12*3)
