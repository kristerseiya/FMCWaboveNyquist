...
if __name__ == "__main__":
    ...
    x_hat_results = np.zeros((n_estimator, len(distance_arr), n_simulation))

    comp_times = np.zeros((n_estimator, len(distance_arr), n_simulation))
    
    
    d_sim_idx_pair = list(range(len(distance_arr)))
    ...
    for d_idx in range(len(distance_arr)):
        ...
        for n in range(n_simulation):
            ...
            for i, estimator in enumerate(estimators):
                ...
                x_hat_results[i,d_idx,n] = x_hat[0]
                done = time.time()
                comp_times[i,d_idx,n] = done-start
                
            print(f"{d_idx+1:d}/{len(distance_arr):d}")
    ...
    np.savez(results_filename, description=args.description, 
             ...
             distance=distance_arr, n_simulation=n_simulation, n_cycle=n_cycle, **dict(zip(estimator_name_keys, x_hat_results)),
